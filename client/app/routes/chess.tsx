import { Chess } from "chess.js";
import { Chessboard } from "react-chessboard";
import { useEffect, useRef, useState } from "react";
import { io } from "socket.io-client";


// const socket = io("http://localhost:5000");
export default function ChessGame() {
    const chessGameRef = useRef(new Chess()); // keeps game state
  const chessGame = chessGameRef.current;
  const baseUrl = "http://localhost:5000";
  
  const [fen, setFen] = useState("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
  let mymove = "";

  //   useEffect(() => {
  //   socket.on("board_update", (data) => {
  //     setFen(data.board);
  //   });
  // },[]);

   function movePiece(move: string, fen: string) {
  fetch("http://localhost:5000/move", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      move: move, // e.g., "e2e4"
      fen: fen,   // current board FEN
    }),
  })
    .then((res) => res.json())
    .then((data) => {
      if (data.success) {
      
        // setFen(data.board);
        // setTurn(data.turn);
      } else {
        console.error("Move failed:", data.error);
      }
    })
    .catch((err) => console.error("Request error:", err));
}



    const onPieceDrop = ({ sourceSquare, targetSquare }: { sourceSquare: string; targetSquare: string }) => {
  // attempt to make the move locally
  const move = chessGame.move({
    from: sourceSquare,
    to: targetSquare,
    promotion: 'q' // optional: always promote to queen
  });

  if (move === null) return false; // illegal move

  // update the board instantly
  setFen(chessGame.fen());

  // send move/FEN to server
  movePiece(move);
  return true;
};

    
    
  const chessboardOptions = {
    id: "main-chessboard",
    position: fen,
    boardOrientation: "white",
    allowDragging: true,
    allowDragOffBoard: false,
    allowDrawingArrows: true,
    showNotation: true,

    // Board Styling
    boardStyle: {
      borderRadius: "8px",
      boxShadow: "0px 4px 10px rgba(0, 0, 0, 0.3)",
      maxWidth: "480px",
      margin: "auto",
    },
    lightSquareStyle: { backgroundColor: "#f0d9b5" },
    darkSquareStyle: { backgroundColor: "#b58863" },

    // Optional events
    onPieceDrop
    // Arrow example
    
    
  };

  return <Chessboard options={chessboardOptions} />;
}

